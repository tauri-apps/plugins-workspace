...
- [`120d0055`](https://github.com/tauri-apps/plugins-workspace/commit/120d00558ebecd4a5fb25c68603d46f6e7b22d2b) ([#3603](https://github.com/tauri-apps/plugins-workspace/pull/3603)) **Breaking:** On Linux, failing to run `xdg-mime` or `update-desktop-database` in `register` and `is_registered` now returns the new `Error::Execute(command, io_error)` variant instead of logging the failure and returning the raw `Error::Io`.
