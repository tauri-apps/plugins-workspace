...
function formatRaw(ctx, value, recurseTimes, typedArray) {
  ...
  if (Reflect.has(value, Symbol.iterator) || constructor === null) {
    ...
    if (Array.isArray(value)) {
      ...
      if (constructor === null) {
        // TODO(wafuwafu13): Implement
        // fallback = TypedArrayPrototypeGetSymbol.toStringTag(value);
...
function formatNamespaceObject(keys, ctx, value, recurseTimes) {
  ...
  for (let i = 0; i < keys.length; i++) {
    try {
      ...
      const pos = String.prototype.lastIndexOf(output[i], " ");
      // We have to find the last whitespace and have to replace that value as
      // it will be visualized as a regular string.
      output[i] =
        String.prototype.slice(output[i], 0, pos + 1) +
...
function getBoxedBase(value, ctx, keys, constructor, tag) {
  ...
  if (isNumberObject(value)) {
    type = "Number";
    primitive = Number.prototype.valueOf(value);
  } else if (isStringObject(value)) {
    type = "String";
    primitive = String.prototype.valueOf(value);
    // For boxed Strings, we have to remove the 0-n indexed entries,
    // since they just noisy up the output and are redundant
    // Make boxed primitive Strings look like such
    Array.prototype.splice(keys, 0, value.length);
  } else if (isBooleanObject(value)) {
    type = "Boolean";
    primitive = Boolean.prototype.valueOf(value);
  } else if (isBigIntObject(value)) {
    type = "BigInt";
    primitive = BigInt.prototype.valueOf(value);
  } else {
    type = "Symbol";
    primitive = Symbol.prototype.valueOf(value);
  }
  ...
  return ctx.stylize(base, String.prototype.toLowerCase(type));
}
...
function createFilteredInspectProxy({ object, keys, evaluate }) {
  ...
  if (object.constructor?.name) {
    Object.defineProperty(obj, "name", { value: object.constructor.name });
  }
  ...
  return new Proxy(new obj(), {
    get(_target, key) {
      ...
      } else if (Array.prototype.includes(keys, key)) {
    ...
    getOwnPropertyDescriptor(_target, key) {
      if (!Array.prototype.includes(keys, key)) {
        return undefined;
      } else if (evaluate) {
    ...
    has(_target, key) {
      return Array.prototype.includes(keys, key);
    },
    ownKeys() {
      return keys;
    },
