...
fn main() {
    ...
    if !commands_dir.exists() {
        create_dir_all(commands_dir).expect("unable to create autogenerated commands dir");
    }
...
description = "This enables the {command} command without any pre-configured accessible paths."
commands.allow = [
    "{command}"
]
...
description = "This denies the {command} command without any pre-configured accessible paths."
commands.deny = [
    "{command}"
]"###,
            command = command,
        ...
        std::fs::write(commands_dir.join(format!("{command}.toml")), toml)
